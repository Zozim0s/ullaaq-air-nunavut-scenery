...
# Ullaaq experimental semantic-terrain support.
#
...
quad_init_level = 3
quad_capacity_high = 8000
quad_capacity_low = 8000
...
def build_dsf(tile, download_queue):
    ...
    if (tile.water_tech == "XP11 + bathy"):
        # Native X-Plane land: lon, lat, elevation, normal-x, normal-y
        dsf_pool_plane = 5 * numpy.ones(dsf_pool_nbr, "int")
    ...
    # Register one native X-Plane terrain definition per Ullaaq class.
    ullaaq_class_to_terrain_idx = {}
    for class_id in sorted(ULLAAQ_TERRAIN_DEFS):
        terrain_idx = len(dico_terrains)
        terrain_key = ("ullaaq_landclass", class_id)
        dico_terrains[terrain_key] = terrain_idx
        ullaaq_class_to_terrain_idx[class_id] = terrain_idx
        textured_tris[terrain_idx] = defaultdict(lambda: array.array("H"))
        bTERT += bytes(ULLAAQ_TERRAIN_DEFS[class_id] + "\0", "ascii")
    ...
    ullaaq_class_counts = defaultdict(int)
    ...
    for tri in range(nbr_tris):
        ...
        if tri_type == 0:
            # Experimental Ullaaq landclass-driven native X-Plane terrain.
            bary_lon = (
            ...
            ullaaq_class_counts[class_id] += 1
            ...
            # The entries for the terrain and texture main dictionaries
            terrain_attributes = (texture_attributes, tri_type)
            is_overlay = False
        ...
        for n in (n1, n3, n2):  # beware of ordering for orientation !
            ...
            if node_hash in textured_nodes:
                ...
                if not tri_type:  # native X-Plane land, no ortho UVs
                    idx_dsfpool = idx_pool
                    dsf_pools[idx_dsfpool].extend(
                        node_icoords[5 * n : 5 * n + 5]
                    )
                else:  # inland water still uses ortho coordinates
                    (s, t) = GEO.st_coord(
                        node_coords[5 * n + 1],
                        node_coords[5 * n],
                        *texture_attributes
